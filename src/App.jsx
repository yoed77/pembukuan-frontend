...
function App() {
  ...
  const fetchData = async () => {
    try {
      const resCat = await fetch('http://192.168.1.32:3001/api/categories');
      const dataCat = await resCat.json();
      setCategories(dataCat);

      const resTrans = await fetch('http://192.168.1.32:3001/api/transactions');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://192.168.1.32:3001/api/transactions', {
        method: 'POST',
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus catatan ini?')) {
      try {
        const response = await fetch(`http://192.168.1.32:3001/api/transactions/${id}`, { method: 'DELETE' });
