...
function App() {
  ...
  // ─── STATE TAMBAHAN UNTUK MULTI-FILTER PENCARIAN RIWAYAT ───────────
  ...
  // 7. LOGIKA FILTER RIWAYAT TRANSAKSI (SINKRON DENGAN FITUR BARU)
  const filteredTransactions = transactions.filter(t => {
    const rawDate = t.date ? t.date.substring(0, 10) : '';
    const transType = t.category_type || t.type || '';
    
    // A. Filter Waktu (Bulan Terpilih VS Rentang Tanggal S/D Tanggal)
    if (filterMode === 'month') {
    ...
    // B. Filter Jenis (Pemasukan / Pengeluaran)
    if (filterType !== 'all') {
  ...
  return (
    <div className="max-w-xl mx-auto bg-gray-100 min-h-screen pb-10 font-sans antialiased text-gray-800">
      ...
      {/* 🔍 PANEL FILTER PENCARIAN RIWAYAT (SUDAH DIUPGRADE DAN DI-CUSTOM) */}
      <div className="m-3 p-4 bg-white rounded-2xl shadow-sm border border-gray-100 space-y-3">
        ...
        {/* Pilihan Metode Tanggal */}
        <div className="grid grid-cols-3 gap-2 items-end">
          <div>
            <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">1. Rentang Waktu</label>
            <select value={filterMode} onChange={(e) => setFilterMode(e.target.value)} className="w-full p-2 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl text-xs font-medium focus:outline-none">
              <option value="month">📅 Ikut Acuan Bulan</option>
              <option value="range">📆 Custom Rentang Tanggal</option>
            </select>
          </div>

          {/* Form Dari Tanggal s/d Tanggal */}
          {filterMode === 'range' ? (
            <>
              <div>
                <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">Dari Tanggal</label>
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full p-1.5 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl text-xs focus:outline-none" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">s/d Tanggal</label>
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full p-1.5 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl text-xs focus:outline-none" />
              </div>
            </>
          ) : (
            <div className="col-span-2 p-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-semibold text-center">
              Menyaring otomatis data berdasarkan bulan acuan di atas
            </div>
          )}
        </div>

        {/* Filter Jenis Transaksi & Kategori & Metode */}
        <div className="grid grid-cols-3 gap-2">
          {/* Tambahan Filter Jenis Transaksi */}
          <div>
            <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">2. Jenis Uang</label>
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full p-2 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl text-xs font-medium focus:outline-none">
          ...
          <div>
            <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">3. Kategori</label>
          ...
          <div>
            <label className="block text-[10px] text-gray-400 font-bold uppercase mb-1">4. Metode</label>
      ...
      <div className="m-3 p-4 bg-white rounded-2xl shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-[10px] px-2 py-0.5 bg-blue-50 text-blue-600 font-bold rounded-full">{filteredTransactions.length} Baris ditemukan</span>
        </div>
